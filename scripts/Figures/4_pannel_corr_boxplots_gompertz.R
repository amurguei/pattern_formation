...
# Panel (b): relative position ratio
m_ratio_day <- lmer(
  ratio ~ genotype + day_sc + (1 | nubbin_id),
  data = boundary_df
)

# Panel (c): distance from original nubbin through time
m_nubbin_day <- lmer(
  d_nubbin ~ genotype * day_sc + (1 | nubbin_id),
  data = boundary_df
)

# Panel (d): distance to contemporaneous perimeter
m_perimeter_day <- lmer(
  d_perimeter ~ genotype + day_sc + (1 | nubbin_id),
  data = boundary_df
)

# Panel (a): probability of being closer to perimeter
m_binary_day <- glmer(
...
ratio_span <- diff(range(boundary_df$ratio, na.rm = TRUE))
if (ratio_span == 0) ratio_span <- 1

ratio_label_y_const <- 15.1

ratio_letter_pos <- letters_ratio %>%
  dplyr::select(genotype, ratio_letter) %>%
  mutate(label_y = ratio_label_y_const)

perimeter_span <- diff(range(boundary_df$d_perimeter, na.rm = TRUE))
if (perimeter_span == 0) perimeter_span <- 1

perimeter_label_y_const <- max(boundary_df$d_perimeter, na.rm = TRUE) +
  0.10 * perimeter_span

perimeter_y_lower <- min(boundary_df$d_perimeter, na.rm = TRUE) -
  0.03 * perimeter_span

perimeter_y_upper <- perimeter_label_y_const +
  0.08 * perimeter_span
...
pD <- ggplot(boundary_df, aes(x = genotype, y = d_perimeter)) +
  ...
  scale_y_continuous(
    limits = c(perimeter_y_lower, perimeter_y_upper),
    expand = expansion(mult = c(0.01, 0.02))
  ) +
  theme_boundary +
  theme(
...
ggsave(
  filename = file.path(plot_dir, "boundary_positioning_composite_4panel_SE_cadet_letters_oneline.png"),
...
ggsave(
  filename = file.path(plot_dir, "boundary_positioning_composite_4panel_SE_cadet_letters_oneline.pdf"),
  plot = figure_boundary_se,
  width = 12,
  height = 9,
  bg = "white"
...
ggsave(
  filename = file.path(plot_dir, "boundary_positioning_composite_4panel_SD_cadet_letters_oneline.png"),
...
ggsave(
  filename = file.path(plot_dir, "boundary_positioning_composite_4panel_SD_cadet_letters_oneline.pdf"),
  plot = figure_boundary_sd,
  width = 12,
  height = 9,
  bg = "white"
...
ggsave(
  file.path(plot_dir, "boundary_panel_a_binary_probability.png"),
  pA,
  width = 7,
  height = 5,
  dpi = 600,
...
ggsave(
  file.path(plot_dir, "boundary_panel_a_binary_probability.pdf"),
  pA,
  width = 7,
  height = 5,
  bg = "white"
...
ggsave(
  file.path(plot_dir, "boundary_panel_b_ratio_letters_oneline.png"),
  pB,
  width = 7,
  height = 5,
  dpi = 600,
...
ggsave(
  file.path(plot_dir, "boundary_panel_b_ratio_letters_oneline.pdf"),
  pB,
  width = 7,
  height = 5,
  bg = "white"
...
ggsave(
  file.path(plot_dir, "boundary_panel_c_time_SE.png"),
  pC_se,
  width = 8,
  height = 5.5,
  dpi = 600,
...
ggsave(
  file.path(plot_dir, "boundary_panel_c_time_SE.pdf"),
  pC_se,
  width = 8,
  height = 5.5,
  bg = "white"
...
ggsave(
  file.path(plot_dir, "boundary_panel_c_time_SD.png"),
  pC_sd,
  width = 8,
  height = 5.5,
  dpi = 600,
...
ggsave(
  file.path(plot_dir, "boundary_panel_c_time_SD.pdf"),
  pC_sd,
  width = 8,
  height = 5.5,
  bg = "white"
...
ggsave(
  file.path(plot_dir, "boundary_panel_d_perimeter_letters_oneline.png"),
  pD,
  width = 7,
  height = 5,
  dpi = 600,
...
ggsave(
  file.path(plot_dir, "boundary_panel_d_perimeter_letters_oneline.pdf"),
  pD,
  width = 7,
  height = 5,
  bg = "white"
...
cat("\nDone. Four-panel boundary-positioning figures saved.\n")