...
## Cropping fix: coord_cartesian(), not scale limits
############################################################
...
perimeter_y_lower <- 0
perimeter_y_upper <- perimeter_label_y_const + 0.10 * perimeter_span
...
pD <- ggplot(boundary_df, aes(x = genotype, y = d_perimeter)) +
  ...
  coord_cartesian(
    ylim = c(perimeter_y_lower, perimeter_y_upper),
    clip = "off"
  ) +
  labs(
    x = "Colony",
...
# ---------------------------------------------------------
# Letter positions: cleaner visual version
# ---------------------------------------------------------

# Panel (b): keep letters inside the plotting area, no floating outliers
...
# Panel (d): closer to previous look, not overly stretched
...
# ---------------------------------------------------------
# Panel (b): ratio boxplot, visually clipped at 16
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Panel (d): perimeter-distance boxplot, restored less-squished look
# ---------------------------------------------------------
